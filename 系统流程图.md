...
不是模型爱调什么就调什么。`plan_turn` 是纯函数：读事实、写 `ExecutionPlan`（action / tools / goal），不改会话、不选货。缺厘米只允许 `collect`；要出图但还没有套装，本步是 `compose`，`goal` 记下延后的 `render`。调完一个工具再规划一次。名单外返回 `planner_blocked`。主持只能看见本步 schema。
